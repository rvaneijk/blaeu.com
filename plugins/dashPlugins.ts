...
export interface DashCache {
  ...
  init(): void;
  registerPlayer(id: string, player: MediaPlayer, source: string): MediaPlayer;
  getPlayer(id: string): MediaPlayer | null;
  getPlayerBySource(source: string): MediaPlayer | null;
  ...
  if (process.client) {
    ...
    const dashCache: DashCache = {
      ...
      registerPlayer(id: string, player: MediaPlayer, source: string): MediaPlayer {
        ...
        errorHandler.debug('DASH player registered', { id, source }, { component: 'DashPlugin' });
        return player;
      },
      ...
      getPlayerBySource(source: string): MediaPlayer | null {
        ...
        errorHandler.debug('Looking for cached player with source', { source }, { component: 'DashPlugin' });
        errorHandler.debug('Available cached sources', { sources: Object.values(this.players).map(p => p.source) }, { component: 'DashPlugin' });
        
        for (const id in this.players) {
          if (this.players[id].source === source) {
            errorHandler.debug('Found cached player for source', { id, source }, { component: 'DashPlugin' });
            // Re-apply cookie prevention before returning
            this.disableCookieStorage(this.players[id].player);
            return this.players[id].player;
          }
        }
        errorHandler.debug('No cached player found for source', { source }, { component: 'DashPlugin' });
        return null;
      },
