    ...
    const handleKeyboardShortcuts = (e) => {
      ...
      if (e.altKey) {
        ...
        switch (e.key) {
          ...
          // Responsible disclosure shortcut (moved to 'd' for disclosure)
          case 'd':
          case 'D':
            e.preventDefault();
            // Toggle responsible disclosure widget
            try {
              const event = new CustomEvent('open-disclosure-widget', {
                detail: { language: document.documentElement.lang === 'nl' ? 'nl' : 'en' }
              });
              document.dispatchEvent(event);
              announceAction('Responsible disclosure toggled');
            } catch (err) {
              console.error('Error toggling responsible disclosure widget:', err);
            }
            break;
