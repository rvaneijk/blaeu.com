...
A single command runs a multi-step automated pipeline:
...
| 7    | Inject Subresource Integrity (SRI) hashes into HTML        |
| 8    | Generate sitemap                                           |
| 9    | Extract CSP hashes and update `customHttp.yml` for Amplify |
| 10   | Sync source code to github                                 |
| 11   | Deploy to AWS Amplify                                      |

## Security

- **CSP Level 3**: Security steps 8–10 implement [CSP Level 3](https://www.w3.org/TR/CSP3/) with `script-src` hash allowlists and SRI verification on all external assets — fully automated on every deploy.
- **Content Security Policy (CSP3)**: Hash-based `script-src`, no `unsafe-inline`; `unsafe-hashes` scoped only to `script-src-attr` and `style-src-attr`
