...
export default {
  ...
  emits: {
     ...
     *   console.log(`Quality changed to ${qualityLabel} (${bitrate}kbps, ${width}x${height})`);
     *   // Update quality indicator UI, track analytics, etc.
     * }
     */
    'quality-change': (qualityInfo) => {
      return qualityInfo && 
             typeof qualityInfo.bitrate === 'number' && 
             typeof qualityInfo.qualityLabel === 'string' &&
             typeof qualityInfo.width === 'number' && 
             typeof qualityInfo.height === 'number';
    }
  },
  ...
  methods: {
    ...
    async initializeVideoPlayer() {
      ...
      try {
        const result = await this.initializePlayer(
          ...
          this.dashSource,
          {
            useCachedPlayer: this.useCachedPlayer,
            emit: this.$emit
          }
        );
    ...
    onVideoError(event) {
      ...
      errorHandler.warning('Video loading error', event, { component: 'VideoPlayer' });
      if (!this.decorative) {
    ...
    onVideoEnded(event) {
      // Handle video ending using recovery composable
      this.videoRecovery.handleVideoEnded(
        this.$refs.videoPlayer,
        this.isPlaying,
        () => this.initializeVideoPlayer()
      );
