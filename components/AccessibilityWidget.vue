...
<template>
  <div 
    :class="[
      ...
      { 'accessibility-widget-open': isOpen },
      positionClass
    ]"
    ref="widgetRef"
  >
    ...
    >
      ...
      <div class="accessibility-widget-content">
        ...
        <div class="accessibility-control">
          <label id="font-family-control-label">{{ currentLanguage === 'nl' ? 'Lettertype' : 'Font Family' }}</label>
        ...
        <!-- Font size controls -->
        <div class="accessibility-control">
          <label for="font-size-control">{{ currentLanguage === 'nl' ? 'Tekstgrootte' : 'Text Size' }}</label>
          <div class="button-group">
            ...
            >
              A<span class="sr-only">Smaller</span>
            </button>
            <button 
              @click="resetFontSize()" 
              class="control-button"
            ...
            >
              A<span class="sr-only">Default</span>
            </button>
            <button 
            ...
            >
              A<span class="sr-only">Larger</span>
            </button>
          </div>
        </div>
        ...
        <!-- Contrast controls -->
        <div class="accessibility-control">
          <label id="contrast-control-label">{{ currentLanguage === 'nl' ? 'Contrast' : 'Contrast' }}</label>
        ...
        <div class="accessibility-control">
          <div class="control-switch">
            ...
            >
            <label for="focus-mode">{{ currentLanguage === 'nl' ? 'Focusmodus' : 'Focus Mode' }}</label>
          </div>
        </div>
        ...
        <div class="accessibility-control">
          <div class="control-switch">
            ...
            >
            <label for="reduce-motion">{{ currentLanguage === 'nl' ? 'Verminder Beweging' : 'Reduce Motion' }}</label>
            <span class="sr-only" id="reduce-motion-desc">Enable to pause all videos (including DASH players) and reduce animations.</span>
          </div>
        </div>
        
        <!-- Position control - only bottom positions for left/right handed users -->
        <div class="accessibility-control">
          <label id="position-control-label">{{ currentLanguage === 'nl' ? 'Widget Positie' : 'Widget Position' }}</label>
          <div class="button-group position-controls-simplified" role="radiogroup" aria-labelledby="position-control-label">
            <button 
            ...
            >
              ...
              <span class="position-label">{{ currentLanguage === 'nl' ? 'Linkshandig' : 'Left-handed' }}</span>
            </button>
            <button 
            ...
            >
              ...
              <span class="position-label">{{ currentLanguage === 'nl' ? 'Rechtshandig' : 'Right-handed' }}</span>
            </button>
          </div>
        </div>
        ...
        <div class="accessibility-control keyboard-shortcuts-hint" @click="showKeyboardShortcuts">
          <div class="flex items-center text-sm text-gray-500 cursor-pointer hover:text-gray-700">
            ...
            <span>{{ currentLanguage === 'nl' ? 'Druk op' : 'Press' }} <kbd class="bg-gray-100 px-1 rounded border border-gray-300 text-xs">Alt</kbd> + <kbd class="bg-gray-100 px-1 rounded border border-gray-300 text-xs">/</kbd> {{ currentLanguage === 'nl' ? 'voor toetsenbordsnelkoppelingen' : 'for keyboard shortcuts' }}</span>
          </div>
        </div>
      </div>
    </div>
...
import KeyboardShortcutsWidget from '~/components/KeyboardShortcutsWidget.vue';
...
const widgetRef = ref(null);                      // Reference to widget DOM element
const currentLanguage = ref('en');                // Default language: 'nl' or 'en'

/**
 * Event handler references for cleanup
 */
let keyboardHandler = null;                       // Keyboard event handler reference
let documentClickHandler = null;                  // Click outside handler reference
...
const toggleWidget = () => {
  ...
  if (process.client) {
    if (isOpen.value) {
      ...
      const cookieElements = document.querySelectorAll('.cookie-policy-widget, .cookie-policy-widget-toggle, .cookie-policy-widget button');
...
const changeFontSize = (direction) => {
  if (direction === 'increase') {
    fontSizeLevel.value = Math.min(fontSizeLevel.value + 1, 3); // Max level is 3
  } else {
    fontSizeLevel.value = Math.max(fontSizeLevel.value - 1, -2); // Min level is -2
  }
  
  // Update global state
  globalState.fontSizeLevel = fontSizeLevel.value;
...
const applyFontSize = () => {
  ...
  document.documentElement.classList.remove('font-size-decrease-2', 'font-size-decrease-1', 'font-size-increase-1', 'font-size-increase-2', 'font-size-increase-3');
...
const toggleFocusMode = () => {
  ...
  const { $announce } = useNuxtApp();
  if ($announce) {
    const message = focusModeEnabled.value 
      ? 'Focus mode enabled' 
      : 'Focus mode disabled';
...
const toggleCaptions = () => {
  ...
  if (pageHasVideos) {
    ...
    if (nuxtApp && nuxtApp.$toggleCaptions) {
...
const setPosition = (newPosition) => {
  ...
  if ($announce) {
    ...
    $announce(`All widgets moved to ${newPosition.replace('-', ' ')} for ${handedness} users`, 'polite');
  }
};

// Helper function to ensure all widgets move to the same side
const moveAllWidgetsToSide = (position) => {
  ...
  nextTick(() => {
    try {
      ...
      if (backToTopBtn) {
        ...
        backToTopBtn.classList.remove('right-6', 'right-5', 'left-6', 'left-5', 'right-aligned', 'left-aligned');
...
const setFontFamily = (fontFamily) => {
  ...
  // Update global state
  globalState.fontFamily = fontFamily;
  ...
  const { $announce } = useNuxtApp();
  if ($announce) {
    $announce(`${fontFamily === 'dyslexic' ? 'Dyslexic friendly' : 'Default'} font enabled`, 'polite');
  }
};
  ...
  if (process.client && window.localStorage) {
    try {
      ...
      if (savedState) {
        ...
        // Only apply settings that haven't already been set from global state
        if (fontSizeLevel.value === 0 && state.fontSizeLevel !== undefined) {
          fontSizeLevel.value = state.fontSizeLevel;
        }
        ...
        if (!focusModeEnabled.value && state.focusMode) {
          focusModeEnabled.value = state.focusMode;
        }
        
        if (!reduceMotion.value && state.reduceMotion) {
          reduceMotion.value = state.reduceMotion;
        }
        ...
        if (captionsEnabled.value !== state.videoCaptions) {
          captionsEnabled.value = state.videoCaptions;
        }
        
        if (currentFontFamily.value === 'default' && state.fontFamily) {
          currentFontFamily.value = state.fontFamily;
        }
        
        if (position.value === 'bottom-right' && state.widgetPosition) {
          position.value = state.widgetPosition;
        }
      }
    } catch (e) {
...
watch(() => globalState.highContrast, (newValue) => {
...
watch(() => globalState.focusMode, (newValue) => {
  if (focusModeEnabled.value !== newValue) {
    focusModeEnabled.value = newValue;
    enhancedFocus.value = newValue; // Keep both in sync for backward compatibility
    applyFocusMode();
  }
});

watch(() => globalState.videoCaptions, (newValue) => {
...
watch(() => globalState.fontFamily, (newValue) => {
...
watch(() => globalState.fontSizeLevel, (newValue) => {
...
onMounted(() => {
  ...
  // Check global state for language preference
  if (globalState.languagePreference) {
    currentLanguage.value = globalState.languagePreference;
  } else if (process.client && navigator && navigator.language) {
    // Use browser language as fallback
    currentLanguage.value = navigator.language.toLowerCase().startsWith('nl') ? 'nl' : 'en';
    // Update global state with the detected language
    globalState.languagePreference = currentLanguage.value;
  }
  ...
  if (globalState.widgetPosition) {
    ...
    if (globalState.widgetPosition === 'bottom-left' || globalState.widgetPosition === 'bottom-right') {
  ...
  globalState.focusMode = focusModeEnabled.value;
  globalState.videoCaptions = captionsEnabled.value;
  globalState.fontFamily = currentFontFamily.value;
  globalState.fontSizeLevel = fontSizeLevel.value;
  globalState.widgetPosition = position.value;
...
onBeforeUnmount(() => {
  ...
  document.documentElement.classList.remove('font-size-decrease-2', 'font-size-decrease-1', 
    'font-size-increase-1', 'font-size-increase-2', 'font-size-increase-3');
  document.documentElement.classList.remove('high-contrast-mode');
...
.accessibility-widget-toggle-open {
  background-color: #005bb8 !important; /* Darker blue to indicate active state */
