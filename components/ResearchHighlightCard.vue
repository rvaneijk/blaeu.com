...
<template>
  <div :class="cardClasses" role="listitem">
    <!-- Image -->
    <div class="h-48 overflow-hidden bg-gray-50 dark:bg-gray-800">
      <img
        :src="item.image"
    ...
    <div class="p-6">
      ...
      <div class="mb-4">
        ...
        <button
          v-if="hasLongIntro"
          class="mt-2 text-brand-grey hover:text-gray-800 dark:hover:text-gray-200 text-sm font-medium transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 rounded"
      ...
      <!-- Action Links -->
      <div v-if="hasLinks" class="flex flex-col action-links-container">
        ...
        <a
          ...
          class="no-external-icon action-link inline-flex items-center gap-2 text-brand-grey hover:text-gray-800 dark:hover:text-gray-200 text-sm font-medium transition-colors duration-200"
        >
        ...
        <template v-for="link in item.links" :key="`archives-${link.url}`">
          <a
            ...
            class="no-external-icon action-link inline-flex items-center gap-2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 text-sm transition-colors duration-200"
          >
            <i class="fa-solid fa-copy fa-xs text-gray-400 dark:text-gray-500"></i>
...
<script setup lang="ts">
  ...
  const cardClasses = computed(() => {
    return `bg-white dark:bg-gray-900 overflow-hidden`
  })
  ...
  // Archive links temporarily hidden pending URL audit (2026-02-21).
  // To restore: replace the function body with:
  //   if (!archiveUrls) return []
  //   return archiveUrls.filter(archive => archive.status !== 'broken')
  // Before restoring: review tmp/archive-urls-2026-02-21.md, pin URLs that should
  // not be auto-updated (add "pinned": true in JSON), then run check-urls.js to fix redirects.
  const getActiveArchives = (_archiveUrls?: ArchiveUrl[]): ArchiveUrl[] => {
    return []
  }
