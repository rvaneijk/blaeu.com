...
const mediaData = ref<MediaContentData | null>(panelExpertData as MediaContentData)
const selectedCategory = ref<string | null>(null)
const showBackToTop = ref(false)

// Helper functions for venue categorization
const isAcademicVenue = (venue: string): boolean => {
  return venue.includes('cpdp') || venue.includes('privacy symposium') || venue.includes('alpine privacy') || 
         venue.includes('data protection conference') || venue.includes('academic') || venue.includes('university')
}

const isProfessionalVenue = (venue: string, publisher: string): boolean => {
  return venue.includes('iapp') || venue.includes('scl') || venue.includes('iab') || 
         publisher.includes('iapp') || publisher.includes('law') || venue.includes('legal')
}

const isPolicyVenue = (venue: string): boolean => {
  return venue.includes('oecd') || venue.includes('parliament') || venue.includes('government') || 
         venue.includes('commission') || venue.includes('policy') || venue.includes('regulatory')
}

const isIndustryVenue = (venue: string): boolean => {
  return venue.includes('google') || venue.includes('mozilla') || venue.includes('meta') || 
         venue.includes('zoom') || venue.includes('industry') || venue.includes('publisher') || 
         venue.includes('summit') || (venue.includes('symposium') && !venue.includes('privacy symposium'))
}

const isForumVenue = (venue: string): boolean => {
  return venue.includes('forum') || venue.includes('workshop') || venue.includes('roundtable') || 
         venue.includes('international') || venue.includes('global') || venue.includes('rightscon')
}

// Get role type from item for filtering (moderator vs panelist)
const getRoleType = (item: Record<string, unknown>): string => {
  const metadata = item.metadata as Record<string, unknown> | undefined
  const role = (metadata?.role as string) || ''
  
  // If the role mentions "Moderator: Eijk R. van", then you were the moderator
  if (role.includes('Moderator: Eijk R. van')) {
    return 'moderator'
  }
  
  // Otherwise, you were a panelist
  return 'panelist'
}

// Get venue type from item for filtering
...
const filteredItems = computed(() => {
  ...
  return allItems.value.filter(item => {
    // Handle role-based filtering (moderator/panelist)
...
const availableCategories = computed(() => {
  ...
  // Count role types (moderator/panelist)
  const roleCounts = allItems.value.reduce((acc, item) => {
    const roleType = getRoleType(item)
    acc[roleType] = (acc[roleType] || 0) + 1
    return acc
  }, {} as Record<string, number>)
  
  // Count venue types (excluding 'other')
  const venueCounts = allItems.value.reduce((acc, item) => {
    const venueType = getVenueType(item)
    if (venueType !== 'other') {
  ...
  // Combine role and venue categories, with specific order
  const allCategories = []
  
  // Add moderator first (after All)
  ...
  // Add panelist second (after moderator)
  ...
  // Add venue types in order of count
...
const handleScroll = (event: Event): void => {
  // Check if scroll is happening in modal (event target) or window
