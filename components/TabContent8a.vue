...
<script setup lang="ts">
  ...
  const availableCategories = computed(() => {
    ...
    if (recordingCount > 0) {
      categories.push({ value: 'has-recording', label: 'Has Recording', count: recordingCount })
    }
  ...
  const filteredItems = computed(() => {
    ...
    return allItems.value.filter(item => getLearningType(item) === selectedCategory.value)
  })
