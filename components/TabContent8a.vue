...
<template>
  <div class="p-1 sm:p-2">
    ...
    <div v-else-if="mediaData" class="grid gap-6">
      <!-- Professional Learning Type Filters -->
      <div class="bg-white p-3 sm:p-4 rounded-lg border border-gray-200">
        <h3 class="text-sm font-medium text-gray-700 mb-3">Filter by learning type:</h3>
      ...
      <div class="space-y-3">
        ...
        <!-- No Results -->
        <div v-else-if="!hasMore && visibleItems.length === 0" class="text-center py-8 text-gray-500">
          <p class="text-lg font-medium mb-2">No learning programs found</p>
          <p class="text-sm">Try selecting a different learning type filter</p>
        </div>
...
import { useMediaContent } from '~/composables/useMediaContent'

// Modal back-to-top positioning composable
const { containerClasses } = useModalBackToTop()

// Media content filtering
const { filterActiveLinks } = useMediaContent()

// Component state
const loading = ref(false)
const error = ref<string | null>(null)
const mediaData = ref<MediaContentData | null>(professionalLearningData as MediaContentData)
const selectedCategory = ref<string | null>(null)
const showBackToTop = ref(false)

// Helper functions for learning type categorization
const isProfessionalTraining = (venue: string, publisher: string, _title: string): boolean => {
  return venue.includes('fpf training') || venue.includes('training program') || 
         venue.includes('digital data flows masterclass') || venue.includes('masterclass') ||
         venue.includes('iab') || publisher.includes('iab') || venue.includes('workshop') ||
         (venue.includes('understanding') && !venue.includes('masterclass')) ||
         publisher.includes('future of privacy forum')
}

const isConferenceSession = (venue: string): boolean => {
  return venue.includes('cpdp') || venue.includes('conference')
}

const isIndustrySession = (venue: string, title: string): boolean => {
  return venue.includes('cpra law') || title.includes('universal opt-outs')
}

const isCertificationProgram = (venue: string, year: string | number): boolean => {
  const yearNum = typeof year === 'string' ? parseInt(year.split('-')[0]) : year
  return yearNum <= 2010 && (venue.includes('cai') || venue.includes('prince2') || 
         venue.includes('exin') || venue.includes('ambi'))
}

const isRegulatorySession = (venue: string, publisher: string, title: string): boolean => {
  return venue.includes('commission') || venue.includes('authority') || 
         publisher.includes('commission') || publisher.includes('authority') ||
         venue.includes('information and privacy commissioner') ||
         title.includes('data clean rooms')
}

// Get learning type from item for filtering
const getLearningType = (item: MediaItem): string => {
  ...
  if (isRegulatorySession(venue, publisher, title)) return 'regulatory'
  if (isIndustrySession(venue, title)) return 'industry'
  if (isConferenceSession(venue)) return 'conference'
  if (isCertificationProgram(venue, year)) return 'certification'
  if (isProfessionalTraining(venue, publisher, title)) return 'training'
...
const allItems = computed(() => {
  ...
  // Filter out items with no active links, then sort by date descending (newest first)
  const activeItems = filterActiveLinks(items)
...
const filteredItems = computed(() => {
  ...
  return allItems.value.filter(item => {
    const learningType = getLearningType(item)
    return learningType === selectedCategory.value
  })
})
...
const availableCategories = computed(() => {
  ...
  const categoryCounts = allItems.value.reduce((acc, item) => {
    const type = getLearningType(item)
  ...
  const categoryLabels = {
    ...
    industry: 'Industry Sessions',
    certification: 'IT Certifications',
    regulatory: 'Regulatory Sessions'
  }
