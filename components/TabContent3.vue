...
<template>
  <div class="p-1 sm:p-2">
    ...
    <div v-else-if="mediaData" class="grid gap-6">
      ...
      <!-- Professional Publications Items -->
      <div class="space-y-3">
...
// Helper function to normalize professional publication categories
const normalizeProfessionalCategory = (category: string): string => {
  ...
  if (categoryLower.includes('abstract')) {
    return 'Abstracts'
  }
  if (categoryLower.includes('article')) {
    return 'Articles'
  }
  if (categoryLower.includes('panel')) {
    return 'Panels'
  }
...
// Publication type filtering - categorize professional publications
const availableCategories = computed(() => {
  ...
  mediaData.value.sections.forEach(section => {
    section.items.forEach(item => {
      const normalizedCategory = normalizeProfessionalCategory(item.category)
      categoryCount[normalizedCategory] = (categoryCount[normalizedCategory] || 0) + 1
    })
  })
  
  // Order by professional publication importance
  const categoryOrder = [
    'Reports',
    'Articles', 
    'Abstracts',
...
onMounted(async () => {
  try {
    ...
    // Use the professional publications JSON data
    mediaData.value = professionalPublicationsData as MediaContentData
