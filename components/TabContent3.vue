...
<script setup lang="ts">
  ...
  const normalizeCategory = (category?: string): string => {
    ...
    if (c === 'blog post') return 'Blog Posts'
    if (c.includes('article')) return 'Articles'
    if (c.includes('report') || c.includes('abstract')) return 'Reports & Abstracts'
    return 'Other'
  }
  ...
  const availableCategories = computed(() => {
    ...
    const categoryOrder = ['Blog Posts', 'Reports & Abstracts', 'Articles']
