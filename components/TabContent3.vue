...
<script setup lang="ts">
  ...
  const normalizeProfessionalCategory = (category?: string): string => {
    ...
    if (categoryLower.includes('report')) return 'Reports'
    if (categoryLower.includes('abstract')) return 'Abstracts'
    if (categoryLower.includes('article')) return 'Articles'
    if (categoryLower.includes('panel')) return 'Panels'
  ...
  const availableCategories = computed(() => {
    ...
    mediaData.value.sections.forEach(section => {
      section.items.forEach(item => {
        const normalized = normalizeProfessionalCategory(item.category)
  ...
  const filteredItems = computed(() => {
    ...
    return allItems.value.filter(item => {
      return normalizeProfessionalCategory(item.category) === selectedCategory.value
    })
  })
