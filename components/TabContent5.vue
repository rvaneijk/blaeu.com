  ...
  <div class="p-1 sm:p-2">
    ...
    <div v-else-if="mediaData" class="grid gap-6">
      ...
      <div class="bg-white p-3 sm:p-4 rounded-lg border border-gray-200">
        <h3 class="text-sm font-medium text-gray-700 mb-3">Filter by publication:</h3>
...
// Media content composable for link filtering
...
// Publication filtering - using venue field for major publications
const availableCategories = computed(() => {
  ...
  mediaData.value.sections.forEach(section => {
    section.items.forEach(item => {
      // Normalize venue names for major publications
      let normalizedVenue = item.venue
      if (item.venue === 'New York Times') {
        normalizedVenue = 'NYT'
      } else if (item.venue === 'Bloomberg') {
        normalizedVenue = 'Bloomberg'
      } else if (item.venue?.includes('POLITICO') || item.venue?.includes('Politico')) {
        normalizedVenue = 'POLITICO'
      } else if (item.venue === 'MLex') {
        normalizedVenue = 'MLEX'
      } else if (item.venue === 'NOS') {
        normalizedVenue = 'NOS'
      }
      
      venueCount[normalizedVenue] = (venueCount[normalizedVenue] || 0) + 1
    })
  })
  
  // Only show the major publications you specified
  const majorPublications = ['NYT', 'Bloomberg', 'POLITICO', 'MLEX', 'NOS']
  
  return majorPublications
    .filter(pub => venueCount[pub] > 0)
    .map(pub => ({
      value: pub,
      label: pub,
      count: venueCount[pub]
    }))
...
// Filtered items based on selected publication
const filteredItems = computed(() => {
  if (!selectedCategory.value) return allItems.value
  
  return allItems.value.filter(item => {
    // Normalize venue for filtering
    let normalizedVenue = item.venue
    if (item.venue === 'New York Times') {
      normalizedVenue = 'NYT'
    } else if (item.venue === 'Bloomberg') {
    ...
    return normalizedVenue === selectedCategory.value
  })
})
...
onMounted(async () => {
  try {
    ...
    // Use the media mentions JSON data
    mediaData.value = mediaMentionsData as MediaContentData
