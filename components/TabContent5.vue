...
<script setup lang="ts">
  ...
  type Outlet = 'mlex' | 'politico' | 'bloomberg' | 'nyt' | 'fd' | 'nos'
  type TaggedItem = MediaItem & { _outlet: Outlet | null }
  type Filter = { type: 'year'; value: string } | { type: 'outlet'; value: Outlet }
  ...
  const matchOutlet = (item: MediaItem): Outlet | null => {
    const p = (item.publisher || '').toLowerCase()
    if (p === 'mlex') return 'mlex'
    if (p === 'politico') return 'politico'
    if (p === 'bloomberg') return 'bloomberg'
    if (p === 'new york times') return 'nyt'
    if (p === 'fd mediagroep') return 'fd'
    if (/^nos\b/.test(p)) return 'nos'
    return null
  }
  ...
  const allItems = computed<TaggedItem[]>(() => {
    ...
    const items: TaggedItem[] = (mediaMentionsData as MediaContentData).sections.flatMap(
      (s: MediaSection) => s.items.map(item => ({ ...item, _outlet: matchOutlet(item) }))
    )
  ...
  const availableFilters = computed<FilterOption[]>(() => {
    ...
    const outletCount: Record<Outlet, number> = {
      mlex: 0,
      politico: 0,
      bloomberg: 0,
      nyt: 0,
    ...
    allItems.value.forEach(item => {
      ...
      if (item._outlet) outletCount[item._outlet]++
    })
    const options: FilterOption[] = []
    const outlets: { value: Outlet; label: string }[] = [
      { value: 'mlex', label: 'MLex' },
    ...
    for (const o of outlets) {
      if (outletCount[o.value] > 0) {
        options.push({
          filter: { type: 'outlet', value: o.value },
          label: o.label,
          count: outletCount[o.value],
        })
      }
    }
    for (const y of ['2025', '2024–2022', '2021–2019', 'Earlier']) {
      if (yearCount[y] > 0)
  ...
  const filteredItems = computed(() => {
    ...
    return allItems.value.filter(item => item._outlet === f.value)
  })
