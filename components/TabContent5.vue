...
<script setup lang="ts">
  ...
  const availableCategories = computed(() => {
    ...
    mediaData.value.sections.forEach(section => {
      section.items.forEach(item => {
        ...
        if (item.venue === 'New York Times') normalizedVenue = 'NYT'
        else if (item.venue?.includes('POLITICO') || item.venue?.includes('Politico'))
          normalizedVenue = 'POLITICO'
        else if (item.venue === 'MLex') normalizedVenue = 'MLEX'
        else if (item.venue === 'Bloomberg') normalizedVenue = 'Bloomberg'
        else if (item.venue === 'NOS') normalizedVenue = 'NOS'
  ...
  const shouldShowYearHeader = (item: MediaItem, index: number): boolean => {
    ...
    return currentYear !== getItemYear(previousItem)
  }
