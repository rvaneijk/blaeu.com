...
<script setup lang="ts">
  ...
  const normalizeSpeakingCategory = (category?: string): string => {
    ...
    if (categoryLower.includes('opening remarks')) return 'Opening Remarks'
    return 'Keynotes'
  }
  ...
  const availableCategories = computed(() => {
    ...
    mediaData.value.sections.forEach(section => {
      section.items.forEach(item => {
        const normalized = normalizeSpeakingCategory(item.category)
  ...
  const filteredItems = computed(() => {
    ...
    return allItems.value.filter(item => {
      return normalizeSpeakingCategory(item.category) === selectedCategory.value
    })
  })
