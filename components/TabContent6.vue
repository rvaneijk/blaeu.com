...
<template>
  <div class="p-1 sm:p-2">
    ...
    <div v-else-if="mediaData" class="grid gap-6">
      <!-- Speaking Engagement Type Filters -->
      <div class="bg-white p-3 sm:p-4 rounded-lg border border-gray-200">
        <h3 class="text-sm font-medium text-gray-700 mb-3">Filter by engagement type:</h3>
...
import legacyContentData from '~/data/legacy-content.json'
...
// Helper function to normalize speaking engagement categories
const normalizeSpeakingCategory = (category: string): string => {
  if (!category) return 'Keynotes'
  
  const categoryLower = category.toLowerCase()
  
  if (categoryLower.includes('opening remarks')) {
    return 'Opening Remarks'
  }
  if (categoryLower.includes('keynote')) {
    return 'Keynotes'
  }
...
// Speaking engagement type filtering - categorize speaking engagements
const availableCategories = computed(() => {
  ...
  mediaData.value.sections.forEach(section => {
    section.items.forEach(item => {
      const normalizedCategory = normalizeSpeakingCategory(item.category)
      categoryCount[normalizedCategory] = (categoryCount[normalizedCategory] || 0) + 1
    })
  })
  
  // Order by speaking engagement importance
  const categoryOrder = [
    'Keynotes',
    'Opening Remarks'
  ]
...
// Filtered items based on selected speaking engagement type
...
onMounted(async () => {
  try {
    ...
    // Use the legacy content JSON data
    mediaData.value = legacyContentData as MediaContentData
