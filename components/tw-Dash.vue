...
export default {
  ...
  methods: {
    ...
    handlePlayerReady(player) {
      ...
      // Preload background video dash manifest (only for video mode)
     ...
     * Only runs when video content is being used (desktop mode)
     * Creates a preload link in the document head
     */
    preloadBackgroundVideo() {
      // Only preload if we should be using video content (desktop/good connection)
      if (!this.shouldPreloadVideo) {
        return;
      }
      ...
      errorHandler.debug('Preloaded background video manifest', null, { component: 'Dash' });
    }
  },
