          ...
          <div v-if="currentLanguage === 'en'" class="prose max-w-none text-gray-800">
            <div class="mb-6">
              ...
              <ul class="custom-bullet-list mt-2 space-y-1">
                <li>
                  Blaeu Privacy Response Team B.V. (herinafter: Team Blaeu) is a private limited
...
If there is an inconsistency between the Dutch and English-language version of these regulations, the Dutch version takes precedence.`
  ...
  onMounted(async () => {
    ...
    document.addEventListener('open-terms-widget', ((event: Event) => {
      ...
      if (customEvent.detail && customEvent.detail.language) {
        currentLanguage.value = customEvent.detail.language as 'en' | 'nl'
      ...
      if (customEvent.detail && customEvent.detail.expandPolicy) {
        isPolicyExpanded.value = true
      }
