        ...
        <div v-if="!isPolicyExpanded" class="policy-summary">
          ...
          <div v-else class="prose max-w-none text-gray-800">
           <div class="mb-6">
            ...
            <p class="text-gray-500 mt-4">Laatst bijgewerkt: 1 maart 20255</p>
            </div>
          </div>
...
// Watch global state for language
watch(() => globalState.termsLanguage, (newValue) => {
...
onMounted(() => {
  ...
  // Get language from global state if available
  if (globalState.termsLanguage) {
    currentLanguage.value = globalState.termsLanguage;
  }
  ...
  document.addEventListener('open-terms-widget', (event) => {
    ...
    if (event.detail && event.detail.language) {
      ...
      // Update global state
      globalState.termsLanguage = currentLanguage.value;
    }
...
// Update global state when language changes
watch(currentLanguage, (newValue) => {
  globalState.termsLanguage = newValue;
});
</script>
...
.terms-widget {
  position: fixed;
  z-index: 54 !important; /* Higher than privacy widget (53), cookie widget (52) and accessibility widget (51) - important to override styles */
...
@media (max-width: 640px) {
  .terms-widget-panel.expanded {
    ...
    z-index: 5500; /* Highest z-index to ensure it's above everything */
    overflow-y: auto; /* Ensure content can scroll */
    padding-top: 4rem; /* Add padding at top to account for nav bar */
  }
  ...
  .terms-widget-panel.expanded .terms-widget-content {
    ...
    overflow-y: auto;
    padding-bottom: 3rem; /* Additional padding to ensure content isn't cut off */
    padding-top: 0.5rem;
  }
}
...
.custom-bullet-list li::before {
  content: '•';
  position: absolute;
  left: 1px; /* Aligns with the "I" in "In short:" */
  color: #000;
  font-weight: bold;
}
</style>