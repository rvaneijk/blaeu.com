  ...
  <div class="bg-gray-50 p-6 rounded-lg shadow-md flex flex-col border-l-4 border-brand-grey" role="listitem">
    ...
    <div class="mt-4">
      <a
        ...
        role="button"
        aria-label="View committees and boards portfolio"
        aria-haspopup="dialog"
        @click.prevent="openCommitteesModal"
        @keydown.enter.prevent="openCommitteesModal"
      >
        <i class="fas fa-users fa-xs text-brand-gold"></i>
...
const committeesStats = computed(() => {
  ...
  const organizations = data.sections.flatMap(section => 
    section.items.map(item => item.venue || '')
  ).filter(Boolean)
  
  const hasW3C = organizations.some(org => 
    org.toLowerCase().includes('w3c') || org.toLowerCase().includes('world wide web')
  )
  
  const hasStandards = organizations.some(org => 
