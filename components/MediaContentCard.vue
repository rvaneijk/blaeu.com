    ...
    <div>
      ...
      <div class="text-sm space-y-2 flex-grow text-gray-600 mb-4">
        <div>
          ...
          <span v-if="item.venue" class="italic">{{ item.venue }}</span><span v-if="item.venue">. </span>
          <span v-if="item.location">{{ item.location }}</span><span v-if="item.location && item.publisher">: </span>
          <span v-if="item.publisher">{{ item.publisher }}</span><span v-if="item.publisher">.</span>
          <span v-if="item.metadata?.timestamp" class="font-mono text-xs"> [{{ item.metadata.timestamp }}]</span>
        </div>
      </div>
      
      <!-- Enhanced action links -->
      <div v-if="hasLinks" class="flex flex-col" style="gap: 0.0625rem !important; line-height: 1.2 !important;">
        <!-- Main links -->
        <a 
        ...
        <template v-for="link in activeLinks" :key="`archives-${link.url}`">
          <a 
...
const activeLinks = computed(() => {
  ...
  return props.item.links.filter(link => link.status === 'active' || !link.status)
})

const hasLinks = computed(() => {
  // Check if there are active main links or any archive links
  const hasActiveMainLinks = activeLinks.value.length > 0
  const hasArchiveLinks = activeLinks.value.some(link => 
    link.archiveUrls && link.archiveUrls.some(archive => archive.status !== 'broken')
  )
  
  return hasActiveMainLinks || hasArchiveLinks
})
