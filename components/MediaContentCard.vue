...
const hasLinks = computed(() => {
  ...
  // Check if there are main links or any archive links
  const hasMainLinks = props.item.links.length > 0
  const hasArchiveLinks = props.item.links.some(link => 
    link.archiveUrls && link.archiveUrls.some(archive => archive.status !== 'broken')
  )
  
  return hasMainLinks || hasArchiveLinks
})
