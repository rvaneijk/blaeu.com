...
<script setup lang="ts">
  ...
  const normalizePublicationCategory = (category?: string): string => {
    ...
    if (categoryLower.includes('conference')) return 'Conference Papers'
    if (categoryLower.includes('book') || categoryLower.includes('chapter'))
      return 'Books & Chapters'
    if (categoryLower.includes('thesis')) return 'Theses'
  ...
  const availableCategories = computed(() => {
    ...
    mediaData.value.sections.forEach(section => {
      section.items.forEach(item => {
        const normalized = normalizePublicationCategory(item.category)
  ...
  const filteredItems = computed(() => {
    ...
    return allItems.value.filter(item => {
      return normalizePublicationCategory(item.category) === selectedCategory.value
    })
  })
