  ...
  <div class="p-1 sm:p-2">
    ...
    <div v-else-if="mediaData" class="grid gap-6">
      ...
      <div class="bg-white p-3 sm:p-4 rounded-lg border border-gray-200">
        <h3 class="text-sm font-medium text-gray-700 mb-3">Filter by publication type:</h3>
...
// Helper function to normalize publication categories
const normalizePublicationCategory = (category: string): string => {
  if (!category) return 'Other Publications'
  
  const categoryLower = category.toLowerCase()
  
  if (categoryLower.includes('journal') || categoryLower.includes('article')) {
    return 'Journal Articles'
  }
  if (categoryLower.includes('conference')) {
    return 'Conference Papers'
  }
  if (categoryLower.includes('book') || categoryLower.includes('chapter')) {
    return 'Books & Chapters'
  }
  if (categoryLower.includes('thesis')) {
    return 'Theses'
  }
  if (categoryLower.includes('report') || categoryLower.includes('abstract')) {
    return 'Reports & Abstracts'
  }
  
  return 'Other Publications'
}

// Publication type filtering - categorize academic publications
const availableCategories = computed(() => {
  ...
  mediaData.value.sections.forEach(section => {
    section.items.forEach(item => {
      const normalizedCategory = normalizePublicationCategory(item.category)
      categoryCount[normalizedCategory] = (categoryCount[normalizedCategory] || 0) + 1
    })
  })
  
  // Order by importance in academic context
  const categoryOrder = [
    'Journal Articles',
    'Conference Papers', 
    'Books & Chapters',
...
// Filtered items based on selected publication type
const filteredItems = computed(() => {
  if (!selectedCategory.value) return allItems.value
  
  return allItems.value.filter(item => {
    const normalizedCategory = normalizePublicationCategory(item.category)
    return normalizedCategory === selectedCategory.value
  })
})
...
const onEnter = (el: Element, done: () => void): void => {
  ...
  const delay = (index % 4) * 120 // 120ms delay between items in a batch
...
onMounted(async () => {
  try {
    ...
    // Use the scholarly works JSON data
    mediaData.value = scholarlyWorksData as MediaContentData
