...
// But avoid direct setter to improve performance
// Use a ref instead of computed to avoid dependency on globalState reactivity
const currentLanguage = ref(globalState.languagePreference);

// Set up two-way binding similar to AccessibilityWidget
...
// Handler function for immediate language changes
const handleLanguageChange = (language) => {
  // Update local state immediately for responsive UI
  currentLanguage.value = language;
  
  // Then update global state if needed
  if (globalState.languagePreference !== language) {
...
defineExpose({
  openWidget: (language) => {
    ...
    if (language && language !== globalState.languagePreference) {
      globalState.languagePreference = language;
    }
    isOpen.value = true;
...
onMounted(() => {
  ...
  // to defer non-critical localStorage operations
  if (process.client) {
    const checkCookiePolicyShown = () => {
      try {
    ...
    scrollHandler = () => {
      ...
      // Determine if user is scrolled down a full window height
    ...
    const startBlinkingToggle = () => {
      ...
      const toggleBlinkingState = () => {
        ...
        // Use the local state variable instead of reading localStorage on every animation frame
        // This greatly improves performance by avoiding frequent localStorage reads
        let shouldStop = hasBeenOpenedBefore.value;
        
        if (shouldStop) {
          // Reset to base state and don't schedule the next toggle
          isBlinking.value = false;
          // Only manipulate the DOM if necessary (when we need to reset styles)
          ...
          if (toggleBtn) {
            ...
            Object.assign(toggleBtn.style, {
              backgroundColor: baseColor,
              transform: '', // Remove inline transform to allow CSS hover effect
              boxShadow: '0 4px 10px rgba(0, 0, 0, 0.2)',
              outline: 'none'
            });
            toggleBtn.innerHTML = '<i class="fa-solid fa-cookie-bite text-xl" aria-hidden="true"></i>';
        ...
        // Use the component state rather than reading from localStorage on every animation frame
        // This significantly improves performance while maintaining the same functionality
        let policyShown = hasBeenOpenedBefore.value;
        ...
        if (!policyShown && !isScrolledDown.value) {
          ...
          if (toggleBtn) {
            ...
            // Batch all style changes in one Object.assign to reduce layout thrashing
            if (isBlinking.value) {
              // ON state - apply all styles at once
              Object.assign(toggleBtn.style, {
              ...
              // OFF state - apply all styles at once
              Object.assign(toggleBtn.style, {
  ...
  const openCookieWidgetHandler = (event) => {
    ...
    // Update language if provided, otherwise use the global languagePreference
    if (event.detail && event.detail.language && event.detail.language !== globalState.languagePreference) {
      globalState.languagePreference = event.detail.language;
    }
