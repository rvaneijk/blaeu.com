...
onMounted(() => {
  ...
  // Listen for privacy policy requests
  document.addEventListener('show-privacy-policy', (event) => {
  ...
  // Listen for position update events from accessibility widget
  document.addEventListener('update-widget-position', (event) => {
  ...
  // Listen for cookie widget open requests - simplified like TermsWidget
  document.addEventListener('open-cookie-widget', (event) => {
...
onBeforeUnmount(() => {
  ...
  document.removeEventListener('open-cookie-widget', null);
...
.cookie-policy-widget-bottom-left {
  bottom: 77px !important; /* Fine-tuned to balance spacing between widgets */
  left: 20px !important;
}
