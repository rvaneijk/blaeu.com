  ...
  <div class="p-1 sm:p-2">
    ...
    <div v-else-if="mediaData" class="grid gap-6">
      ...
      <div class="bg-white p-3 sm:p-4 rounded-lg border border-gray-200">
        <h3 class="text-sm font-medium text-gray-700 mb-3">Filter by committee type:</h3>
        <div class="flex flex-wrap gap-2">
          <button
            ...
            @click="handleCommitteeTypeFilter(null)"
          >
            All ({{ totalFilteredItems }})
          </button>
          <button
            v-for="type in availableCommitteeTypes"
            :key="type.value"
            :class="[
              'px-3 py-1 rounded-full text-sm font-medium transition-colors duration-200',
              selectedCommitteeType === type.value 
                ? 'bg-brand-grey text-white' 
                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
            ]"
            @click="handleCommitteeTypeFilter(type.value)"
          >
      ...
      <div class="space-y-3">
        ...
        >
          <template v-for="(item, index) in visibleItems" :key="`container-${item.id}`">
            ...
            <!-- Committee Content Card with Animation Data -->
            <MediaContentCard 
              :item="item" 
...
const selectedCommitteeType = ref<string | null>(null)
...
const availableCommitteeTypes = computed(() => {
  ...
  mediaData.value.sections.forEach(section => {
    section.items.forEach(item => {
      ...
      // Group similar venues together
      let groupKey: string
      if (venue.toLowerCase().includes('alpine privacy')) {
        groupKey = 'alpine-privacy'
      } else if (venue.toLowerCase().includes('brussels privacy')) {
        groupKey = 'brussels-privacy'
      } else if (venue.toLowerCase().includes('w3c')) {
        groupKey = 'w3c'
      } else if (venue.toLowerCase().includes('dublin privacy')) {
        groupKey = 'dublin-privacy'
      } else {
        groupKey = 'other'
      }
      typeCount[groupKey] = (typeCount[groupKey] || 0) + 1
    })
  })
  
  const typeLabels: Record<string, string> = {
    'alpine-privacy': 'Alpine Privacy Days',
    'brussels-privacy': 'Brussels Privacy Symposium',
    'w3c': 'W3C Workshops',
    'dublin-privacy': 'Dublin Privacy Symposium',
    'other': 'Other Events'
  }
...
// Filtered items based on selected committee type
const filteredItems = computed(() => {
  if (!selectedCommitteeType.value) return allItems.value
  ...
  return allItems.value.filter(item => {
    ...
    return groupKey === selectedCommitteeType.value
  })
})
...
// Handle committee type filter changes
const handleCommitteeTypeFilter = (committeeType: string | null): void => {
  selectedCommitteeType.value = committeeType
...
const onEnter = (el: Element, done: () => void): void => {
  ...
  const delay = (index % 3) * 150 // 150ms delay between items in a batch
...
onMounted(async () => {
  try {
    ...
    // Later this could be replaced with an API call: await useMediaContent().loadMediaData('committees-boards')
    mediaData.value = committeesData as MediaContentData
