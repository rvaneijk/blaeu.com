...
<script setup lang="ts">
  ...
  const normalizeYearCategory = (year: number): string => {
    if (year >= 2024) return '2026–2024'
    if (year >= 2021) return '2023–2021'
    return 'Earlier'
  }
  ...
  const availableCategories = computed(() => {
    ...
    mediaData.value.sections.forEach(section => {
      section.items.forEach(item => {
        const year = item.date ? new Date(item.date).getFullYear() : (item.year as number)
        const bucket = normalizeYearCategory(year)
        yearCount[bucket] = (yearCount[bucket] || 0) + 1
      })
    })
    const yearOrder = ['2026–2024', '2023–2021', 'Earlier']
    return yearOrder
      .filter(y => yearCount[y] > 0)
      .map(y => ({ value: y, label: y, count: yearCount[y] }))
  })
  ...
  const filteredItems = computed(() => {
    ...
    return allItems.value.filter(item => {
      const year = item.date ? new Date(item.date).getFullYear() : (item.year as number)
      return normalizeYearCategory(year) === selectedCategory.value
    })
  })
  ...
  onMounted(() => {
    mediaData.value = committeesBoardsData as MediaContentData
