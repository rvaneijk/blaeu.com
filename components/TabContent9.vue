    ...
    <div v-else-if="mediaData">
      ...
      <div class="mb-6 text-sm text-gray-500 dark:text-gray-400">
        ...
        <a
          ...
          @click.prevent="handleCommitteeTypeFilter(null)"
        >
          All ({{ totalAllItems }})
        </a>
        <template v-for="type in availableCommitteeTypes" :key="type.value">
          ...
          <a
            ...
            @click.prevent="handleCommitteeTypeFilter(type.value)"
          >
...
<script setup lang="ts">
  ...
  const getVenueGroup = (venue?: string): string => {
    const v = (venue || '').toLowerCase()
    if (v.includes('alpine privacy')) return 'alpine-privacy'
    if (v.includes('brussels privacy')) return 'brussels-privacy'
    if (v.includes('w3c')) return 'w3c'
    if (v.includes('dublin privacy')) return 'dublin-privacy'
    return 'other'
  }
  ...
  const availableCommitteeTypes = computed(() => {
    ...
    mediaData.value.sections.forEach(section => {
      section.items.forEach(item => {
        const groupKey = getVenueGroup(item.venue)
  ...
  const filteredItems = computed(() => {
    ...
    return allItems.value.filter(item => getVenueGroup(item.venue) === selectedCommitteeType.value)
  })
