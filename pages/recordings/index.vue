  ...
  <div class="min-h-screen bg-white dark:bg-gray-900 flex flex-col">
    ...
    <main id="main-content" class="pt-12 flex-1">
      <div class="max-w-2xl mx-auto px-4 py-8 text-lg">
        ...
        <!-- Filter -->
        <nav v-if="availableFilters.length > 0" aria-label="Filter recordings by year" class="mb-6">
          <div class="bg-[#f3f3f0] dark:bg-gray-800 px-6 py-5">
            ...
            <ul class="flex flex-wrap gap-2 text-sm">
              ...
              <template v-for="opt in availableFilters" :key="opt.value">
                <li class="flex items-center gap-2">
                  ...
                  <a
                    ...
                    @click.prevent="selectedFilter = opt.value"
                  >
                    {{ opt.label }} ({{ opt.count }})
                  </a>
                </li>
        ...
        <div aria-label="Recordings">
          <template v-for="(item, index) in filteredRecordings" :key="item.id">
            <h2
              v-if="index === 0 || item.year !== filteredRecordings[index - 1].year"
...
<script setup lang="ts">
  ...
  function primaryLink(item: MediaItem): MediaLink | null {
    ...
    return links.find(l => l.type === 'video' || l.type === 'recording') ?? links[0]
  }
  ...
  const availableFilters = computed<FilterOption[]>(() => {
    ...
    for (const item of recordings) {
      const bucket = normalizeYearCategory(item.year)
      counts[bucket] = (counts[bucket] ?? 0) + 1
    }
    return ['2026', '2025', '2024–2022', 'Earlier']
      .filter(b => counts[b] > 0)
      .map(b => ({ value: b, label: b, count: counts[b] }))
  })

  const filteredRecordings = computed(() => {
    if (!selectedFilter.value) return recordings
    return recordings.filter(item => normalizeYearCategory(item.year) === selectedFilter.value)
  })
