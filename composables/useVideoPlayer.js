...
export const useVideoPlayer = () => {
  ...
  const initializePlayer = async (videoElement, dashSource, options = {}) => {
    ...
    if (options.useCachedPlayer && window.dashCache && typeof window.dashCache.getPlayerBySource === 'function') {
      const cachedPlayer = window.dashCache.getPlayerBySource(dashSource)
      if (cachedPlayer) {
        errorHandler.debug('CACHE HIT: Using dashCache player', { source: dashSource }, { component: 'useVideoPlayer' })
  ...
  const setupDashPlayer = async (dashjs, videoElement, dashSource, options) => {
    ...
    try {
      ...
      if (options.useCachedPlayer && window.dashCache && typeof window.dashCache.registerPlayer === 'function') {
        try {
          const playerId = 'dash-player-' + Date.now()
          window.dashCache.registerPlayer(playerId, player.value, dashSource)
          errorHandler.debug('Player registered in advanced cache', { playerId }, { component: 'useVideoPlayer' })
        } catch (err) {
  ...
  const extractQualityInfo = (newQuality) => {
    ...
    if (bitrateInfo && bitrateInfo[newQuality]) {
      ...
      qualityInfo.qualityLabel = qualityLabels[newQuality] || `Quality ${newQuality}`
    }
