...
interface UseModalBackToTopReturn {
  ...
  bottomPosition: Readonly<Ref<string>>
}
...
export const useModalBackToTop = (): UseModalBackToTopReturn => {
  ...
  // Bottom position: above cookie widget when not acknowledged
  const bottomPosition = computed(() => (globalState.cookiePolicyAcknowledged ? '20px' : '90px'))
