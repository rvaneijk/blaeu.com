...
import type { MediaContentData, MediaItem, YearSection, LinkType, MediaType } from '~/data/schema/media-content.schema'
 ...
 */
const createGetMediaTypeIcon = () => (mediaType: string): string => {
 ...
 */
const createOrganizeByYear = () => (items: MediaItem[]): YearSection[] => {
  ...
  return Array.from(yearMap.entries())
    .map(([year, items]) => ({
      year,
      count: items.length,
      items: items.sort((a, b) => new Date(b.metadata?.eventDate || '').getTime() - new Date(a.metadata?.eventDate || '').getTime())
    }))
 ...
 */
const createFilterActiveLinks = () => (items: MediaItem[]): MediaItem[] => {
 ...
 */
const createFilterItems = () => (items: MediaItem[], query: string): MediaItem[] => {
 ...
 */
const createConvertLegacyContent = () => (legacyItem: Record<string, unknown>): MediaItem => {
  ...
  return {
    ...
    description: legacyItem.intro.length > 200 ? legacyItem.intro.substring(0, 200) + '...' : legacyItem.intro,
