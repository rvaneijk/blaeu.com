...
interface GlobalState extends AccessibilityState {
  currentTab: number
  darkMode: boolean
  ...
  cookiePolicyAcknowledged: boolean
...
let initialState: GlobalState = {
  ...
  activeWidget: null,
  // Cookie policy acknowledgment tracking
  cookiePolicyAcknowledged: false,
}
...
if (process.client && isStorageAvailable()) {
  try {
    ...
    if (savedState) {
      ...
      Object.assign(initialState, {
        darkMode: Boolean(parsedState.darkMode),
  ...
  // Load cookie policy acknowledgment from localStorage
  try {
    initialState.cookiePolicyAcknowledged = localStorage.getItem('cookie-policy-shown') === 'true'
  } catch (_e) {
    // Ignore errors reading cookie policy state
  }

  // Migrate legacy 'dark-mode' key into accessibility-state
  try {
    const legacyDarkMode = localStorage.getItem('dark-mode')
    if (legacyDarkMode !== null) {
      initialState.darkMode = legacyDarkMode === 'true'
      localStorage.removeItem('dark-mode')
    }
  } catch (_e) {
    // Ignore errors during dark mode migration
  }
}
