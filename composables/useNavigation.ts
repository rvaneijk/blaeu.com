...
function setupLifecycleManagement(
  ...
  const isTestEnvironment = typeof global !== 'undefined' && 
    ((global as typeof globalThis & { process?: { env?: Record<string, string> } }).process?.env?.NODE_ENV === 'test' ||
     (global as typeof globalThis & { process?: { env?: Record<string, string> } }).process?.env?.VITEST ||
     typeof (global as typeof globalThis & { expect?: unknown }).expect === 'function' ||
     typeof (global as typeof globalThis & { describe?: unknown }).describe === 'function' ||
     typeof (global as typeof globalThis & { test?: unknown }).test === 'function' ||
     typeof (global as typeof globalThis & { it?: unknown }).it === 'function')

  if (isTestEnvironment) {
    // In test environment, skip lifecycle hooks to prevent warnings
    return
  }
